...
class Main extends BD_Controller {
    ...
    public function menu_get()
    {
        ...
        if ($id === NULL)
        {
            $getUser = $this->Crud->readData('id,nama,harga,diskon,kategori,image','menu')->result();
            if ($getUser)
            {
                // Set the response and exit
                $output = [
        ...
        if($id){
            ...
            $getUserById = $this->Crud->readData('id,nama,harga,diskon,kategori,image','menu',$where)->result();
    ...
    public function cart_get($user)
    {
        if($user){
            ...
            if($getUserById){
                $gettotal = $this->db->query("SELECT SUM(subtotal) AS total FROM keranjang WHERE user='".$user."'")->row_array();
                // var_dump($gettotal['total']);die();
    ...
    public function pesananpribadi_get($user)
    {
        if($user){
            ...
            if($getUserById){
                $gettotal = $this->db->query("SELECT SUM(subtotal) AS total FROM pesanan WHERE user='".$user."'")->row_array();
